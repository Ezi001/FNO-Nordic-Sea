...
from utils.data_torch import build_dataloader
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", type=str, default="FNO2d", choices=["FNO2d", "Unet"])
...
def main() -> None:
    ...
    spline_dataset = ZarrSplineDataset(
    "data/splines_train.zarr"
    )

    spline_loader = DataLoader(
        spline_dataset,
    ...
    eval_dataset = (eval_state[:, 0], 
                    eval_state, 
                    #eval_forcing
    )

    print("Starting training loop...")
    train_output = train_cfo(method, 
                            spline_loader, 
                            train_args, 
